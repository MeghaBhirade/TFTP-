...
int send_request(int sock_fd, int req, char *file)
{
	...
	{
		printf("Acknowledgement received for request \n");
		return 1;
	}
}
		...
		else if (retval)
		{
			...
			if (packet.rpack.opcode == DATA)
			{
				if (packet.dpack.block_num == (ack_pack.block_num + 1))
				{
					...
					printf("Sending the acknowledgement for %d\n", packet.dpack.block_num);
				}
				else
					printf("Re-sending the acknowledgement for %d\n", packet.dpack.block_num);
...
void put_file_to_server(int sock_fd, int fd)
{
	...
	while (1)
	{
		...
		while (1)
		{
			...
			printf("Re-sending the packet %d\n", data_pack.block_num);
		}
	}
}
...
int main()
{
	...
	while (1)
	{
		...
		/* Error print */
			...
			printf("First connect to the server\n");
